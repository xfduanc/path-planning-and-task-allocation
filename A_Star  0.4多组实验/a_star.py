...
class AStar:
    ...
    def RunAndSaveImage(self, ax, plt, index, group):
        ...
        while (loop >0):#True:
            ...
            if self.IsEndPoint(p):
                self.open_set = []
                self.close_set = []
                return self.BuildPath(p, ax, plt, start_time, index)
    ...
    def SaveImage(self, plt):
        ...
        filename = './' + self.fileDir+ "/" + '{} {} {} {:x<3d}'.format(str(self.map.size), str(self.group), str(self.index+1) , millis%1000) + '.png'
    ...
    def clear(self):
        self.time.clear()
        self.distance.clear()
    def BuildPath(self, p, ax, plt, start_time, index):
        path = []
        end = p
        ...
        end_time = time.time()
        self.time.append('{:.2f}'.format(end_time-start_time))
